...
Template.addcategory.helpers({
	getCategories: function() {
	    ...
	    var mapChildren = function(cate, level) {
		    ...
		    results.push({_id: cate._id, title: prefix + cate.title});
...
Template.updatecategory.helpers({
	getCat: function(){
		var id =this._id;
		var cats = categories.findOne({_id:id});
		if(cats.parent==0){
			var parentTitle="No Parent";
		}else{
			var parentTitle=categories.findOne({_id:cats.parent}).title;
		}
