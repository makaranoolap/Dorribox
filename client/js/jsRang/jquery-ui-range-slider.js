 ...
 */
(function(jQuery) {
  return jQuery.widget('uiExtension.rangeSlider', jQuery.ui.slider, {
    _create: function() {
      var self;
      self = this;
      self.options.values = [];
      ...
      jQuery(self.options.ranges).each(function(k, v) {
        ...
        self.options.rangeValues.push(v);
        jQuery(self.element).append('<div class="ui-slider-range ui-widget-header range-bg-' + v.id.toString() + '" style="left: ' + (100 * v.startValue / self.options.max).toString() + '%; width: ' + 100 * (v.endValue - v.startValue) / self.options.max + '%; background: none repeat scroll 0% 0% ' + v.color + ';" ><div class="range-label"></div></div>');
      });
      return self._super();
    },
    _parseRangeValues: function(obj) {
      var parsed;
      parsed = jQuery.extend(true, {
        id: (new Date).getTime(),
        startValue: 0,
        endValue: 1,
    ...
    _refreshInfoBars: function() {
      var self;
      self = this;
      return jQuery(self.handles).each(function(k, v) {
    ...
    _rageUiHash: function(event, index) {
      ...
      range = jQuery.grep(this.options.ranges, function(value) {
    ...
    _renderInfoBar: function(event, index) {
      ...
      text = "";
      ui = self._rageUiHash(event, index);
      ui.label = jQuery('.range-label', ui.handlers.rangeBar);
      if (jQuery.isFunction(self.options.rangeLabel)) {
    ...
    _getHandlers: function(index) {
      ...
      handlers = {};
      self = this;
      handlers.currentHandler = jQuery(self.handles[index]);
      handlerSelector = '.range-handler-' + handlers.currentHandler.data('id');
      handlers.leftHandler = jQuery(handlerSelector + '.left:first', self.element);
      handlers.rightHandler = jQuery(handlerSelector + '.right:first', self.element);
      handlers.rangeBar = jQuery('.range-bg-' + handlers.currentHandler.data('id'), self.element);
